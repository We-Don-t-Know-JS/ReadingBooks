...
"scope"가 *Engine*이 어떻게 변수값을 찾는지에 대한 정의라고 했다. 현재의 스코프인지 중첩스코프 내에 포함되었는지.

**Lexical Scope**라는 모델이 있고 몇 개의 언어에서 아직 사용되는 **Dynamic Scope**가 있다.

## Lex-time

표준언어 컴파일러를 lexing (aka, tokenizing)이라고 부른다.
