...
다른 c나 java의 정적 타입 프로그래머는 인정하지 않겠지만 ES5.1부터 타입이라는것이 생겼다.

자바스크립트에서 엔진과 개발자 둘다 42와 '42'의 차이를 인식하고 서로 다르게 처리 될 것이라 생각한다.

숫자 42에서 '2'를 갖고 싶다면 타입변경을 하고 해야한다. (coerce)

정확하게 하지 않으면 의도치 않은 이상한 동작을 하게 된다.
...
다른 타입들은 다 위와 같이 '정상적'으로 동작하지만 null은 이상하게 작동한다.

```js
typeof null === "object"; // true
```

이전에 개발된 legacy가 많으니 앞으로도 수정되기는 어렵. 사용하기 위해서는 아래와 같이 혼합하여 사용 할 수 있다.
...
변수에는 타입이 없고 -- **값에 타입이 있다**. 변수는 아무 값이나 언제든 가지고 있을 수 있기 때문.

그 이유는 JS는 타입 강제하는게 없기 때문에 숫자를 저장한 이후 문자를 저장해도 문제 없다.
...
JavaScript has seven built-in *types*: `null`, `undefined`,  `boolean`, `number`, `string`, `object`, `symbol`. They can be identified by the `typeof` operator. (NULL 제외)
